...
const EventForm = ({ type, data,setOpen }: { type: "create" | "update", data?: any,setOpen: Dispatch<SetStateAction<boolean>> }) => {
    ...
    return (
        <form className='flex flex-col gap-8' onSubmit={onSubmit}>
            ...
            <div className='flex flex-row flex-wrap justify-between gap-2'>
                <InputField label='Title' name='title' type='text' defaultValue={data?.title} register={register} error={errors.title} className='md:w-1/4' />
            ...
            <div className='flex flex-row flex-wrap justify-between gap-2'>
                <InputField label='Date' name='date' type='date' defaultValue={data?.date} register={register} error={errors.date} className='md:w-1/4' />
                <InputField label='Start Date' name='start_date' type='date' defaultValue={data?.start_date} register={register} error={errors.start_date} className='md:w-1/4' />
                <InputField label='End Date' name='end_date' type='date' defaultValue={data?.end_date} register={register} error={errors.end_date} className='md:w-1/4' />
